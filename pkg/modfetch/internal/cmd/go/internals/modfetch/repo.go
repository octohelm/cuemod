...
import (
	...
	"github.com/octohelm/cuemod/pkg/modfetch/internal/cmd/go/internals/modfetch/codehost"
	"github.com/octohelm/cuemod/pkg/modfetch/internal/cmd/go/internals/par"
	"github.com/octohelm/cuemod/pkg/modfetch/internal/cmd/go/internals/vcs"
	web "github.com/octohelm/cuemod/pkg/modfetch/internal/cmd/go/internals/web"
...
// file.
//
// The Lookup function takes a module path and returns a RepoRoot representing
...
func lookupDirect(path string) (Repo, error) {
	...
	if rr.VCS.Name == "mod" {
		// Fetch module from proxy with base URL rr.RepoRoot.
...
func (l *loggingRepo) Versions(prefix string) (tags []string, err error) {
	defer logCall("RepoRoot[%s]: Versions(%q)", l.r.ModulePath(), prefix)()
	return l.r.Versions(prefix)
}

func (l *loggingRepo) Stat(rev string) (*RevInfo, error) {
	defer logCall("RepoRoot[%s]: Stat(%q)", l.r.ModulePath(), rev)()
	return l.r.Stat(rev)
}

func (l *loggingRepo) Latest() (*RevInfo, error) {
	defer logCall("RepoRoot[%s]: Latest()", l.r.ModulePath())()
	return l.r.Latest()
}

func (l *loggingRepo) GoMod(version string) ([]byte, error) {
	defer logCall("RepoRoot[%s]: GoMod(%q)", l.r.ModulePath(), version)()
...
func (l *loggingRepo) Zip(dst io.Writer, version string) error {
	...
	defer logCall("RepoRoot[%s]: Zip(%s, %q)", l.r.ModulePath(), dstName, version)()
