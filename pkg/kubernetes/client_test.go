...
func TestClient(t *testing.T) {
	...
	allGroupVersionKinds, _ := c.AllListableGroupVersionKinds()
		...
		t.Run("Apply & Delete", func(t *testing.T) {
			...
			t.Run("Apply", func(t *testing.T) {
				...
				list, err := c.ListAll(
					...
					client.InNamespace(r.Metadata.Namespace),
					client.MatchingLabels(map[string]string{
						manifest.LabelRelease: r.Metadata.Name,
					}),
				)
				NewWithT(t).Expect(err).To(BeNil())
	...
	t.Run("client", func(t *testing.T) {
		...
		NewWithT(t).Expect(err).To(BeNil())
		NewWithT(t).Expect(list.Items).NotTo(BeEmpty())
	})
}
